...
for( var modelName in models ){//根据数据模式建立数据模型
		mongoose.model( modelName , new Schema( models[ modelName ] ));//设置modal,Schema参数为对象
}

/*传入模型名（类型）获取到相应的模型
* global.dbHandle.getModel("user")
* */
module.exports={
...
var _getModel=function( modelName ){
	return mongoose.model( modelName );//获取modal
}
