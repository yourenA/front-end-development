    ...
    var app = {
        init: function () {
            document.addEventListener('DOMContentLoaded', function () {
                //当前this为window
                points = document.querySelectorAll('.bottom-menu li');
        ...
        bindBtnClick: function () {
            for (var i = 0; i < points.length; i++) {
                (function (i) {
                    points[i].addEventListener("touchend", function (event) {
                        event.stopPropagation();
                        pageNow = i + 1;
                        app.setPageNow();
                        app.transform.call(viewport, -i * pageWidth);/*如果不使用call(viewport)则在app.transform中this指向不正确*/
                    }, false);
                })(i)
            }
        ...
        setPageNow: function () {
            if (currentPoint != -1) {
                points[currentPoint].className = '';/*设置上一个按钮className 为空*/
            }
            currentPoint = pageNow - 1;
            points[currentPoint].className = 'active';
        },
        ...
        bindTouchEvent: function () {
            ...
            document.addEventListener("touchmove", function (e) {
                ...
                if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX)>50 && deltaY <40) {
                    moveLength = deltaX;
